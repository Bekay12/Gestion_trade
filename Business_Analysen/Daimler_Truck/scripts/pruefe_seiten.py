...
from rechnung_daimler import ROH, REIHE_ROH                  # noqa: E402
...
def main() -> int:
    ...
    for posten, jahre in REIHE_ROH.items():
        for j, (wert, bericht, seite) in jahre.items():
            if wert == 0:
                continue           # "--" gedruckt (keine Ausschuettung 2022), nichts zu belegen
            n += 1
            if not belegt(wert, bericht, seite):
                fehler.append(f"REIHE {posten} {j}: {wert} nicht auf {bericht} S. {seite}")
    for f in fehler:
        print("[SEITE]", f)
