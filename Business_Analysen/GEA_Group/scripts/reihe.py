...
# Der Halbjahresbericht druckt Halbjahresspalten (H1 2026, H1 2025). In der Jahresreihe
# waeren das halbe Jahre unter voller Jahreszahl; er wird deshalb getrennt gelesen.
AUSGESCHLOSSEN = {"gea-hj-2026"}
...
ZUSATZ_ZEILEN = {"tnk": {"investitionen": ["Vessel acquisitions"]},
                 ...
                 # GEA: das Modell nennt diese Zeilen nicht; sie stehen in der rechten Spalte
                 # der Kapitalflussrechnung und heissen in jedem Jahrgang gleich.
                 "gea": {"leasing": ["Payments from lease liabilities", "Repayment of lease liabilities",
                                     "Payments for lease liabilities"],
                         "dividenden": ["Dividend payments"],
                         "rueckkauf": ["Payments for acquisition of treasury shares"]},
        ...
        for p, jahre in posten.items():
            ...
            for jahr, les in sorted(jahre.items()):
                ...
                reihen[k][p][jahr] = {"wert": wert, "roh": roh, "bericht": bericht, "seite": seite,
                                      ...
                                      "summe": " + " in etikett}
    # XBRL nur fuer SEC-Emittenten; GEA reicht nicht bei der SEC ein.
    xbrl_datei = os.path.join(ROOT, "data", "xbrl_reihen.json")
    xbrl = json.load(open(xbrl_datei)) if os.path.exists(xbrl_datei) else {}
    abgleich = []
