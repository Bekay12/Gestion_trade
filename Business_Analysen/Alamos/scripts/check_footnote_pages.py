...
# Die Zitiermakros dieses Projekts. Alamos reicht Form 40-F ein, nicht
# 10-K, und die inhaltlichen Dokumente sind deren Anhaenge; deshalb je ein
# Makro fuer Lagebericht (MDA), Abschluss (FS), Zwischenbericht (ZB),
# Zwischenabschluss (ZBA), Einberufungsschreiben (Circ) und Webquelle.
# Die einstelligen Makros tragen nur die Seite, die zweistelligen Jahr und
# Seite. Ohne diesen Ausdruck fand der Pruefer null Zitationen und meldete
# schweigend "alles in Ordnung" - ein Wachhund, der nichts sieht, ist
# gefaehrlicher als keiner.
_CALL = re.compile(
    r"\\quelle(MDA|FS|Web)\{([^}]*)\}\{([^}]*)\}"
    r"|\\quelle(ZBA|ZB|Circ)\{([^}]*)\}")
