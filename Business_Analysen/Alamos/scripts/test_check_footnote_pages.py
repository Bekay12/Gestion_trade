...
class TestParseCitations(unittest.TestCase):
    ...
    def test_findet_gb_aufruf_mit_zeile(self):
        with tempfile.TemporaryDirectory() as tmp:
            self._write(tmp, "01-x.tex", "Text.\\quelleMDA{2024}{2} Weiter.\n")
            hits = parse_citations(tmp)
        self.assertEqual(hits, [("MDA 2024 S.2", "01-x.tex", 1)])

    def test_findet_mehrere_aufrufe_auf_verschiedenen_zeilen(self):
        with tempfile.TemporaryDirectory() as tmp:
            self._write(tmp, "01-x.tex",
                        "Erste.\\quelleMDA{2024}{2}\nZweite.\\quelleMDA{2024}{5}\n")
            hits = parse_citations(tmp)
    ...
    def test_ignoriert_kommentarzeile(self):
        with tempfile.TemporaryDirectory() as tmp:
            self._write(tmp, "01-x.tex",
                        "% FAKTENBASIS: \\quelleMDA{2024}{2}\nEcht.\\quelleMDA{2024}{5}\n")
            hits = parse_citations(tmp)
        self.assertEqual(hits, [("MDA 2024 S.5", "01-x.tex", 2)])
    ...
    def test_mehrere_aufrufe_in_einer_zeile(self):
        with tempfile.TemporaryDirectory() as tmp:
            self._write(tmp, "01-x.tex",
                        "\\quelleMDA{2023}{14}\\quelleFS{2024}{13}\\quelleZB{13}\n")
            hits = parse_citations(tmp)
