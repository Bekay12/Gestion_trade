...
def extract_best_parameters(csv_path: str = 'signaux/optimization_hist_4stp.csv') -> Dict[str, Tuple[Tuple[float, ...], Tuple[float, ...], Tuple[float, float]]]:
    ...
    try:
        ...
        for _, row in best_params.iterrows():
            ...
            if all(k in row.index for k in per_keys):
                thresholds = tuple(float(row[k]) for k in per_keys)
            else:
                thresholds = (50.0, 0.0, 0.0, 1.2, 25.0, 0.0, 0.5, 4.20)

            buy = float(row['Seuil_Achat']) if 'Seuil_Achat' in row.index else 4.20
            sell = float(row['Seuil_Vente']) if 'Seuil_Vente' in row.index else -0.5
            globals_thresholds = (buy, sell)
        ...
        try:
            ...
            for symbol, stock_data in chunk_data.items():
                ...
                try:
                    ...
                    domaine = "ℹ️Inconnu!!"
                signal, last_price, trend, last_rsi, volume_mean, score = get_trading_signal(prices, volumes, domaine, domain_coeffs=best_parameters.get(domaine, None))
                if signal != "NEUTRE":
                    signals.append({
                        'Symbole': symbol,
                        'Signal': signal,
