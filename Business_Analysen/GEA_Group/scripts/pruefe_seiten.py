...
from rechnung_gea import ROH                                     # noqa: E402
...
def main() -> int:
    ...
    kz = json.load(open(os.path.join(ROOT, "data", "kennzahlen_seite.json")))
    for j, posten in kz.items():
        for p, v in posten.items():
            n += 1
            if not belegt(v["wert"], v["bericht"], v["seite"]):
                fehler.append(f"KENNZAHL {p} {j}: {v['wert']} nicht auf {v['bericht']} S. {v['seite']}")
    for f in fehler:
        print("[SEITE]", f)
