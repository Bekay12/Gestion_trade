...
PDF_MUSTER = {
    ...
    # Halbjahresbericht: Fusszeile "G E A H 1 2 02 6 6" -> nach dem Jahr folgt die Seite
    "gea-hj-2026": ("letzte", re.compile(r"(?i)^GEAH1\d{4}(\d{1,3})$")),
...
def zerlege_pdf(quelle: str, ziel: str):
    ...
    for block in t.split("\f"):
        ...
        kandidaten = {"letzte": zeilen[-3:][::-1], "erste": zeilen[:3],
                      "beide": zeilen[:4] + zeilen[-4:][::-1]}[bereich]
        treffer = "?"
        for z in kandidaten:
